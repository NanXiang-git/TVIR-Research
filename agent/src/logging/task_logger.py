...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # Color the logger name (tvir_agent)
...
def bootstrap_logger() -> logging.Logger:
    """Configure the tvir_agent logger with consistent formatting"""

    global logger

    # Configure tvir_agent logger
    tvir_agent_logger = logging.getLogger("tvir_agent")

    # Check if logger already has handlers to prevent duplicate configuration
    if tvir_agent_logger.handlers:
        logger = tvir_agent_logger
        return tvir_agent_logger
    ...
    tvir_agent_logger.addHandler(handler)
    tvir_agent_logger.setLevel(logging.DEBUG)

    # Disable propagation to prevent duplicate logging from root logger
    tvir_agent_logger.propagate = False
...
class TaskLog:
    ...
    def log_step(
        ...
        if "Tool Call Start" in step_name:
            ...
            icon = "🤖 "
        elif "Main Agent" in step_name or "Report Workflow" in step_name:
            icon = "👑 "
    ...
    def save(self):
        ...
        filename = f"{self.log_dir}/{self.task_id}.json"
