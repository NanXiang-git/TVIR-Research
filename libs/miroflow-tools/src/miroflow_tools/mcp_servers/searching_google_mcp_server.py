...
async def google_search(
    ...
    Args:
        ...
        gl: Country context for search. Influences regional results priority:
            - 'us': United States - Use for most international topics and global news
            - 'cn': China - Use for China-specific topics
        hl: Google interface language. Affects snippet language preference:
...
# @mcp.tool()
async def scrape_website_jina(url: str) -> str:
...
async def scrape_website(url: str) -> str:
    """This tool is used to scrape a website for its content using SERPER API.

    Args:
        url: The URL of the website to scrape.
    Returns:
        The scraped website content as plain text.
    """
    # Validate URL format
    if not url or not url.startswith(("http://", "https://")):
        return f"Invalid URL: '{url}'. URL must start with http:// or https://"
    ...
    # Retry logic for SERPER
    retry_count = 0
    max_retries = 3

    while retry_count < max_retries:
        try:
            # Call SERPER scraping API
            ...
            response = requests.post(
                scrape_url, headers=headers, data=payload, timeout=60
            )
            response.raise_for_status()
            ...
            if content is None or content == "":
                raise RuntimeError("No content retrieved from SERPER.")
            ...
            retry_count += 1
            if retry_count >= max_retries:
                return f"[ERROR]: Timeout Error: Request timed out while scraping '{url}' after {max_retries} attempts."
            ...
            retry_count += 1
            if retry_count >= max_retries:
                return f"[ERROR]: Request Error: Failed to scrape '{url}' after {max_retries} attempts: {str(e)}"
            await asyncio.sleep(4**retry_count)

        except Exception as e:
            retry_count += 1
            if retry_count >= max_retries:
                return f"[ERROR]: scrape_website failed after {max_retries} attempts: {str(e)}"
            await asyncio.sleep(4**retry_count)

    return "[ERROR]: Unknown error occurred in scrape_website tool, please try again."
