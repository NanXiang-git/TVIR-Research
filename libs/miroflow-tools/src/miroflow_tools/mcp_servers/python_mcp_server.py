...
async def download_file_from_sandbox_to_local(
    ...
    try:
        ...
        if TASK_ID:
            tmpfiles_dir = os.path.join("results", TASK_ID, "charts")
        else:
            tmpfiles_dir = os.path.join(LOGS_DIR, "tmpfiles")
        ...
        local_file_path = os.path.join(tmpfiles_dir, f"{local_filename}")
        ...
        # Extract the relative path starting from 'charts/'
        if "charts" in local_file_path:
            # Find the position of 'charts' and get everything from there
            charts_index = local_file_path.find("charts")
            relative_path = local_file_path[charts_index:]
        else:
            # Fallback to just the filename if 'charts' is not in the path
            relative_path = local_filename

        return f"File downloaded successfully to: {relative_path}"
    except Exception as e:
        error_details = str(e)[:MAX_ERROR_LEN]
