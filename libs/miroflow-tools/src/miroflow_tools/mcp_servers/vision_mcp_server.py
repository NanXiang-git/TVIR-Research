...
@mcp.tool()
async def visual_question_answering(image_url: str, question: str) -> str:
    """Ask question about an image or a video and get the answer with a vision language model.

    Args:
        image_url: The URL of the image. Supports images (jpg, png, webp, bmp, tiff).
    ...
    while retry < max_retries:
        try:
            ...
            retry += 1
            if retry >= max_retries:
                return f"[ERROR]: Visual question answering failed during API call: {e}"
            await asyncio.sleep(5 * (2**retry))
