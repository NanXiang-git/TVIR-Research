...
def make_serper_request(
    payload: Dict[str, Any], headers: Dict[str, str], type: str = "search"
) -> requests.Response:
    """Make HTTP request to Serper API with retry logic."""
    response = requests.post(f"{SERPER_BASE_URL}/{type}", json=payload, headers=headers)
...
def google_image_search(
    ...
    """
    Tool to perform image searches via Serper API and retrieve image results.

    It is able to retrieve image search results with metadata such as image URL,
    title, source, and dimensions.
    ...
    Args:
        ...
        num: Number of image results to return (default: 5)
    ...
    Returns:
        Dictionary containing image search results and metadata. Thumbnail URLs and Google URLs are removed to reduce response size.
    """
    # Check for API key
    ...
    try:
        ...
        response = make_serper_request(payload, headers, type="images")
        data = response.json()

        # Remove thumbnail and googleUrl fields from each image result
        if "images" in data:
            for item in data["images"]:
                item.pop("thumbnailUrl", None)
                item.pop("thumbnailWidth", None)
                item.pop("thumbnailHeight", None)
                item.pop("googleUrl", None)

        response_data = decode_http_urls_in_dict(data)
...
def google_search(
    ...
    try:
        ...
        # Make the API request
        response = make_serper_request(payload, headers, type="search")
        data = response.json()
